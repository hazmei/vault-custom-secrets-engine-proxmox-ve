...
- **Password mode is verified ONLY on `pve-manager/9.2.14/a1480fa6b8d899cb`.** Every other probe in `docs/PVE_PROBES.md`, and the project's declared target, is 9.2.10 — which has NO password evidence. Password P0 is still partial/open. This is ENFORCED, not advisory: role write refuses `mode=password` unless `GET /version` reports exactly `version=9.2.14` AND `repoid=a1480fa6b8d899cb` (`path_roles.go` Step 7c), and `creds/:role` re-checks the live build before generating a password (`path_creds.go` Step 3b) since the cluster can be upgraded after the role is written. A role write that passes the gate still returns the verified-build warning. Renew and revoke are deliberately NOT gated, so an upgrade breaks new issuance while existing password leases keep renewing and revoking. The gate compares the `version`/`repoid` fields of `GET /version`, which reproduce the `pve-manager` build string (recorded for 9.2.10 in Probe 0; a 9.2.14 `/version` body is not yet recorded). Do not describe password mode as verified on 9.2.10.
