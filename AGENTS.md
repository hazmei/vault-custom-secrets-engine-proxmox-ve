...
**Token-mode lifecycle:** The issuance sequence below is token-mode-specific;
password mode creates the password on the user in the same `POST /access/users`
call and does not create an API token. Renewal and revocation remain shared.
Issuance and mid-create compensation are token-mode-specific; in password mode
the credential is live from `POST /access/users`, so every post-create failure
must delete the user.
